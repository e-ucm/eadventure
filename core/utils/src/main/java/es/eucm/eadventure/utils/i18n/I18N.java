...
import es.eucm.eadventure.utils.clazz.ClassLoaderUtils;
 ...
 * to an integer will result in an {@link java.lang.IllegalArgumentException}
 * </p>
 * <p>
...
public abstract class I18N {
	 ...
	 *            <code>Class</code> holding the messages.
	 */
	private static void loadMessages(final String baseName, final Class<?> clazz, Locale locale) {
		synchronized(clazz){
	...
	private static void assignField(Field field, Object value){
		...
		try {
			 ...
			 * instance if the class is not public) then change the accessible attribute of
			 * the field before trying to set the value.
			 */
			boolean isAccessible = (field.getDeclaringClass().getModifiers() & Modifier.PUBLIC) != 0;
			if (!isAccessible){
				/*
				 ...
				 * silently log it and continue. This means that the field will (most likely) be
				 * un-initialized and will fail later in the code and if so then we will see
				 * both the NPE and this error.
				 */

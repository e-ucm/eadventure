...
abstract public class SwingUtilities {
	...
	static private void showExceptionDialogEDT(final Throwable exp) {
		if (exceptionDialog == null) {
			exceptionDialog = new JDialog(findActiveFrame(),
					Messages.exception_dialog_title, false);
			...
			exceptionDialog.add(new JScrollPane(exceptionEditorPane),
					BorderLayout.CENTER);
			...
			JButton closeBtn = new JButton(
					Messages.exception_dialog_button_close);
		...
		String expType = I18N.bind(
				(exp instanceof Error) ? Messages.exception_dialog_type_error
						: Messages.exception_dialog_type_exception, exp
						.getClass().getName());
		sb.append("<h3>" + expType + "<a name='a"
				+ (exceptionsSoFar.size() + 1) + "'></a>" + "</h3>");
	...
	static private void fillStackTrace(StringBuilder buf, Throwable exp) {
		...
		buf.append("<b>").append(
				I18N.bind(Messages.exception_dialog_message, exp
						.getLocalizedMessage())).append("</b>\n");
		...
		if (cause != null) {
			...
			buf.append("<b>").append(
					I18N.bind(Messages.exception_dialog_caused_by, className))
					.append("</b>\n");

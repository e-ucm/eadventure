...
/**
 * This is PUBLIC DOMAIN code from
 * http://sourceforge.net/projects/iharder/
 * Adapted by manuel.freire@fdi.ucm.es
 *
...
public class FileDrop {
	...
	public FileDrop(final PrintStream out, final Component c,
		...
		dropListener = new DropTargetListener() {
			...
			public void dragEnter(DropTargetDragEvent evt) {
				...
				// If it's a Swing component, save its border
				...
				if (isDragOk(out, evt)) {
					...
					evt.rejectDrag();
					log(out, "FileDrop: event rejected [de].");
			...
			public void dropActionChanged(DropTargetDragEvent evt) {
				...
				if (isDragOk(out, evt)) { //evt.acceptDrag( DnDConstants.ACTION_COPY_OR_MOVE );
					...
					evt.rejectDrag();
					log(out, "FileDrop: event rejected [dac].");
				} // end else: drag not ok
	 ...
	 * @since 1.0
	 */
	public static boolean remove(PrintStream out, Component c, boolean recursive) { // Make sure we support
		if (supportsDnD()) {
	...
	public static class Event extends EventObject {
		...
		private final static long serialVersionUID = 987124987L;

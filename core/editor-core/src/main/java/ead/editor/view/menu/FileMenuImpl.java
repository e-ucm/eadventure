...
public class FileMenuImpl extends MenuImpl implements FileMenu {
	...
	/**
	 * This makes the last-visited directory 'sticky'. An alternative
	 * would have been to save it to a properties file...
	 */
	...
	private static FileFilter ead2LoadFileFilter 
			= new EAdFileFilter(".eap", "data[.]xml", "EAdventure 2 project files");
	private static FileFilter ead2SaveFileFilter 
		...
		mOpen.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				File f = chooseFile(null, Messages.file_menu_open_title,
						...
						true, JFileChooser.FILES_ONLY, ead2LoadFileFilter);
				if (f != null) {
		...
		mImport.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ae) {
				...
				File d = chooseFile(null, Messages.file_menu_import_save_title,
						...
						Messages.file_menu_import_title_error, 
						Messages.file_menu_import_message_error, 
						false, JFileChooser.FILES_ONLY, ead2SaveFileFilter);				
				if (f != null && d != null) {
					controller.getProjectController().load(f.getAbsolutePath());
     ...
     */
    public static File chooseFile(Component p, 

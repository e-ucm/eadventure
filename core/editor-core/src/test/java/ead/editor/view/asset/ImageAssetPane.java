...
import ead.editor.view.asset.FileDrop.Listener;
...
public class ImageAssetPane extends javax.swing.JPanel {
	...
	private GrowableScrollPane jspPreview;
	...
	private ZoomableImagePanel jpCanvas = new ZoomableImagePanel();
	...
	public static class GrowableScrollPane extends JScrollPane {
		...
		private void fixSizes() {
			...
			System.err.println("parent of SP size: " + pd.width + "x"
					+ pd.height);
			Dimension cd = getViewport().getPreferredSize();
			System.err.println("child wants size: " + pd.width + "x"
					+ pd.height);
			pd.width = Math.min(cd.width + 4, pd.width);
			pd.height = Math.min(cd.height + 4, pd.height);
			System.err.println("setting parent SP size to " + pd.width + "x"
					+ pd.height);
			setPreferredSize(pd);
			getParent().doLayout();
		}
	...
	public ImageAssetPane() {
		initComponents();
		jspPreview = new GrowableScrollPane();
		jspPreview.setViewportView(jpCanvas);
		jpCanvasContainer.add(jspPreview, new GridBagConstraints(0, 0, 1, 1, 0,
				0, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
				new Insets(0, 0, 0, 0), 0, 0));
		jspPreview.setParent(jpView);
	...
	private void initComponents() {
		...
		jlDescription.setText("159 - imagen PNG, 1024x30 pixels, 148000 bytes");
		...
		jpCanvasContainer.setLayout(new java.awt.GridBagLayout());
		gridBagConstraints = new java.awt.GridBagConstraints();
		gridBagConstraints.gridx = 0;

...
/**
 * The renderer class for the VarValCondition type
 * @author Roberto Tornero
 * 
 */
public class VarValConditionRenderer extends ConditionRenderer{

	private static final long serialVersionUID = -7147354725486380499L;

	/**
	 * Constructor for the renderer of the VarValCondition type
	 */	
	public VarValConditionRenderer(ConditionsController controller, VarValCondition vcondition, boolean not){
	...
	/**
	 * Private method that returns the name of condition variable as a label component 
	 */
	private Component getSubjectRender() {

		return new JLabel(((VarValCondition)condition).getVar().getName());
	}
	...
	private Component getVerbRender() {
		...
		if( ((VarValCondition)condition).getOperator().equals(Operator.GREATER) )
			return new JLabel(">");

		else if( ((VarValCondition)condition).getOperator().equals(Operator.GREATER_EQUAL) )
			return new JLabel(">=");

		else if( ((VarValCondition)condition).getOperator().equals(Operator.EQUAL) ) 
			return new JLabel("=");

		else if( ((VarValCondition)condition).getOperator().equals(Operator.LESS_EQUAL) ) 
			return new JLabel("<=");

		else if( ((VarValCondition)condition).getOperator().equals(Operator.LESS) ) 
			return new JLabel("<");
	...
	/**
	 * Private method that returns the value of condition variable as a label component 
	 */
	private Component getDirectComplementRender() {

		return new JLabel(""+((VarValCondition)condition).getVal());
	}

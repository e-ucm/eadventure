...
public class AdventureHandler extends DefaultHandler {
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes) {
		if (qName.equals("param")) {
			subparserStack.push(new ParamSubparser(elementStack.peek(),attributes));
		} else if (qName.equals("resources")) {
			subparserStack.push(new ResourcesSubparser((EAdElement) elementStack.peek(), attributes));
		} else if (qName.equals("bundle")) {
			EAdResources resources = ((EAdElement) elementStack.peek()).getResources();
			BundleSubparser bundleSubparser = new BundleSubparser(resources, attributes);
			subparserStack.push(bundleSubparser);
		} else if (qName.equals("asset")) {
			AssetSubparser assetSubparser = new AssetSubparser(attributes);
			subparserStack.peek().addChild(new EntryData(attributes.getValue("id"), assetSubparser.getAsset()));
			subparserStack.push(assetSubparser);
			elementStack.push(assetSubparser.getAsset());
		} else if (qName.equals("element")) {
			ElementSubparser elementSubparser = new ElementSubparser((EAdElement) elementStack.peek(), attributes, classParam);
			...
			ListSubparser listSubparser = new ListSubparser((EAdElement) elementStack.peek(),attributes);
			...
			MapSubparser mapSubparser = new MapSubparser((EAdElement) elementStack.peek(), attributes);
	...
	public void endElement(String uri, String localName, String qName) {
		if (qName.equals("element") || qName.equals("element-ref") || qName.equals("map") || qName.equals("asset") || qName.equals("list"))
			elementStack.pop();

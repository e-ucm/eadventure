...
//	private EAdActorReferenceImpl buttonReference; 
	...
	public BasicScene(String id, EAdChapter chapter) {
		...
		this.panielReference.addBehavior(EAdMouseEventImpl.MOUSE_RIGHT_CLICK,
				showQuestionEffect);
		...
		bse.getResources().addAsset(bse.getInitialBundle(),
				EAdBasicSceneElement.appearance, is);
		...
		bse.addBehavior(EAdMouseEventImpl.MOUSE_ENTERED,
				new EAdChangeAppearance("id", bse, bid));
		bse.addBehavior(EAdMouseEventImpl.MOUSE_EXITED,
				new EAdChangeAppearance("id", bse, bse.getInitialBundle()));
		bse.addBehavior(EAdMouseEventImpl.MOUSE_LEFT_CLICK,
				new EAdChangeFieldValueEffect("id", new EAdFieldImpl<Boolean>(
						timer, EAdTimerImpl.VAR_STARTED), new BooleanOperation(
						"id", EmptyCondition.TRUE_EMPTY_CONDITION)));
		// this.getSceneElements().add(bse);

		EAdTimerEvent timerEvent = new EAdTimerEventImpl("id", timer);
		bse.getEvents().add(timerEvent);
		timerEvent.addEffect(TimerEvent.TIMER_ENDED, new EAdMoveSceneElement(
				"id"));
		// new EAdChangeVarValueEffect("var",
		// panielReference.getVars().getVar(EAdSceneElementVars.VAR_X), new
		// LiteralExpressionOperation("id", "10")));

		EAdCutscene cutscene = new EAdCutscene("id");
		// cutscene.getBackground().getResources().addAsset(cutscene.getInitialBundle(),
		// EAdBasicSceneElement.appearance, new
		// ImageImpl("@drawable/slide1.png"));
		...
		slide.getBackground()
				...
				.addAsset(slide.getBackground().getInitialBundle(),
						EAdBasicSceneElement.appearance,
						new ImageImpl("@drawable/Slide1.png"));
		cutscene.addSlide(slide);

		EAdSlide slide2 = new EAdSlide("id");
		slide2.getBackground()
				.getResources()
				.addAsset(slide2.getBackground().getInitialBundle(),
						EAdBasicSceneElement.appearance,
						new ImageImpl("@drawable/slide2.png"));
		cutscene.addSlide(slide2);

		cutscene.setUpForEngine(chapter);

		this.getBackground().addBehavior(EAdMouseEventImpl.MOUSE_LEFT_CLICK,
				new EAdChangeScene("id", cutscene, EAdTransition.DISPLACE));

		initBall();
		/*
		 * initShowText(); initMoveActor(); initReferences();
		 * initOtherEffects();
		 * 
		 ...
		 * EAdHighlightActorReference highLight = new
		 * EAdHighlightActorReference( behavior, "highlight");
...
//		showQuestionEffect.setUpNewInstance();
	...
	private void initBall() {
		...
		ball.getResources().addAsset(ball.getInitialBundle(),
				EAdBasicSceneElement.appearance,
				new ImageImpl("@drawable/soccer-ball.png"));
...
//
//		EAdChangeFieldValueEffect setVar = new EAdChangeFieldValueEffect("i",
//				dirX, new LiteralExpressionOperation("1", "1"));
//		button.addBehavior(EAdMouseEventImpl.MOUSE_LEFT_CLICK, setVar);
//		setVar = new EAdChangeFieldValueEffect("i", dirY,
//				new LiteralExpressionOperation("1", "1"));
//		button.addBehavior(EAdMouseEventImpl.MOUSE_LEFT_CLICK, setVar);
//
//		LiteralExpressionOperation leX = new LiteralExpressionOperation(
//				"id",
//				"[0] + (abs(400*(1+[1]) - [0]*[1]) min abs(300*(1+[2]) - [3]*[2]))*[1] - 50*[1]",
//				ball.getVars().getVar(EAdSceneElementVars.VAR_X), dirX, dirY,
//				ball.getVars().getVar(EAdSceneElementVars.VAR_Y));
//		LiteralExpressionOperation leY = new LiteralExpressionOperation(
//				"id",
//				"[3] + (abs(400*(1+[1]) - [0]*[1]) min abs(300*(1+[2]) - [3]*[2]))*[2] - 50*[2]",
//				ball.getVars().getVar(EAdSceneElementVars.VAR_X), dirX, dirY,
//				ball.getVars().getVar(EAdSceneElementVars.VAR_Y));
//		EAdMoveSceneElement move = new EAdMoveSceneElement(id, ball, leX, leY,
//				MovementSpeed.FAST);
...
//
//		setVar = new EAdChangeFieldValueEffect("i", dirX,
//				new LiteralExpressionOperation("1", "0-[0]", dirX));
//		setVar.setCondition(new ORCondition(
//				new VarValCondition(ball.getVars().getVar(
//						EAdSceneElementVars.VAR_X), 51, Operator.LESS_EQUAL),
//				new VarValCondition(ball.getVars().getVar(
//						EAdSceneElementVars.VAR_X), 749, Operator.GREATER_EQUAL)));
//		ce.addEffect(ConditionedEvent.CONDITIONS_MET, setVar);
//		setVar = new EAdChangeFieldValueEffect("i", dirY,
//				new LiteralExpressionOperation("1", "0-[0]", dirY));
//		setVar.setCondition(new ORCondition(
//				new VarValCondition(ball.getVars().getVar(
//						EAdSceneElementVars.VAR_Y), 51, Operator.LESS_EQUAL),
//				new VarValCondition(ball.getVars().getVar(
//						EAdSceneElementVars.VAR_Y), 549, Operator.GREATER_EQUAL)));
//		ce.addEffect(ConditionedEvent.CONDITIONS_MET, setVar);
//
...
//		buttonActor.getResources().addAsset(id, EAdBasicActor.appearance,
//				new ImageImpl("@drawable/grab.png"));
//		EAdConditionEvent event = new EAdConditionEventImpl("conditionEvent");
//		event.setCondition(new VarValCondition(new FloatVar("f"), 1.0f,
//				VarCondition.Operator.GREATER_EQUAL));
...
//		buttonReference.setScale(0.8f);
	...
	private void initPanielActor() {
		...
		for (int i = 1; i <= 8; i++)
			animation
					.addFrame(new Frame("@drawable/paniel_wlr_0" + i + ".png"));
		 ...
		 * shape.setColor(EAdColor.BLACK); shape.setHeight(200);
		 * shape.setWidth(200);
		 * panielActor.getResources().addAsset(panielActor.getInitialBundle(),
		...
		actionText.getResources().addAsset(actionText.getInitialBundle(),
		...
		// Behavior
		EAdBehavior behavior = new EAdBehaviorImpl(panielReference.getId()
				+ "_behavior");
		// addEffects(behavior);

		EAdChangeFieldValueEffect changeVar = new EAdChangeFieldValueEffect(
				"changevar");
//		changeVar.addVar(new FloatVar("f"));
//		LiteralExpressionOperation leo = new LiteralExpressionOperation("idop",
//				"[0] + 1", new FloatVar("f"));
//		changeVar.setOperation(leo);
		...
		EAdActorActionsEffect showActions = new EAdActorActionsEffect(
				panielReference.getId() + "_showActions", panielReference);
	...
	private void initOrientedActor() {
		...
		anim.setDrawable(Orientation.N, new ImageImpl("@drawable/going_up.jpg"));
	...
	/*
	 * showTextEffect = new EAdShowText(null, "id3"); EAdString string = new
	 * EAdString("stringId"); EAdTextImpl textImpl = new
	 * EAdTextImpl(showTextEffect, "idText");
	 * 
	 * CaptionImpl caption = new CaptionImpl();
	 * 
	 * 
	 * caption.setMaximumHeight(200); caption.setMaximumWidth(400);
	 * caption.setMinimumHeight(200); caption.setMinimumWidth(100);
	 * showTextEffect.setText(textImpl);
	 * 
	 * textImpl.setText(string);
	 * 
	 * textImpl.setFont(EAdFont.REGULAR); textImpl.setTextColor(new
	 * EAdBorderedColor(EAdColor.BLACK, new EAdColor(80, 80, 80)));
	...
	//
	// move1.setTargetCoordiantes(new LiteralExpressionOperation("id", "50"),
	// new LiteralExpressionOperation("id", "400"));
	//
	// move2 = new EAdMoveSceneElement("move2");
	// move2.setTargetCoordiantes(new LiteralExpressionOperation("id", "400"),
	// new LiteralExpressionOperation("id", "400"));
	//
	// move3 = new EAdMoveSceneElement("move3");
	//
	// move3.setTargetCoordiantes(new LiteralExpressionOperation("id", "50"),
	// new LiteralExpressionOperation("id", "50"));
	//
	// move4 = new EAdMoveSceneElement("move4");
	// move4.setTargetCoordiantes(new LiteralExpressionOperation("id", "500"),
	// new LiteralExpressionOperation("id", "500"));
	...
	private void initChangeScreen() {
		...
		space2.getBackground()
				.getResources()
				.addAsset(space2.getBackground().getInitialBundle(),
						EAdBasicSceneElement.appearance,
						new ImageImpl("@drawable/Creditos.jpg"));

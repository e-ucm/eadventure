...
public class AndroidBasicHUD extends BasicHUDImpl {
	...
	public AndroidBasicHUD(MenuHUD menuHUD,
			...
			ValueMap valueMap, StringHandler stringHandler,
			PlatformConfiguration platformConfiguration,
			AssetHandler assetHandler) {
		super(menuHUD, gameObjectFactory, gameState, gameObjectManager,
				mouseState, valueMap, stringHandler, assetHandler);
	...
	public void render(EAdCanvas<?> canvas) {
		...
		if (mouseState.getMouseX() != -1 && mouseState.getMouseY() != -1) {
			...
			c.drawBitmap(((BitmapCanvas) graphicContext).getBitmap(), new Rect(
					mouseState.getMouseX() - 50, mouseState.getMouseY() - 100,
					mouseState.getMouseX() + 50, mouseState.getMouseY()), rect,
					null);

...
public class SoundMp3 extends Sound {
	...
	public void playOnce() {
		...
		try {
			...
			decodedFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, // Encoding
																				// to
																				// use
			...
			audioInputStream = AudioSystem.getAudioInputStream(decodedFormat,
					ais);
			...
			if (line != null) {
				...
				try {
					...
					int nBytesRead;
					while (!stop
							&& (nBytesRead = audioInputStream.read(data, 0,
									data.length)) != -1) {

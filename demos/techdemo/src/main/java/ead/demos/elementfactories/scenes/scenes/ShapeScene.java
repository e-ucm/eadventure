...
public class ShapeScene extends EmptyScene {
	public ShapeScene() {
		...
		EAdSceneElement e = EAdElementsFactory
				...
				.createSceneElement(rectangleAsset1, rectangleAsset2,
						x + 20, margin);
		...
		EAdDrawable asset1 = shapeFactory.getElement(
				ShapeFactory.ShapeType.CIRCLE_SHAPE, size, size,
				new Paint(ColorFill.YELLOW, ColorFill.BROWN, 4));
		EAdDrawable asset2 = shapeFactory.getElement(
				ShapeFactory.ShapeType.CIRCLE_SHAPE, size, size,
				new Paint(ColorFill.ORANGE, ColorFill.BROWN, 2));
		getSceneElements()
				.add(EAdElementsFactory
						.getInstance()
						.getSceneElementFactory()
		...
		EAdDrawable asset31 = shapeFactory.getElement(
				ShapeFactory.ShapeType.TRIANGLE_SHAPE, size, size,
				new Paint(ColorFill.BLUE, ColorFill.BROWN));
		EAdDrawable asset32 = shapeFactory.getElement(
				ShapeFactory.ShapeType.TRIANGLE_SHAPE, size, size,
				new Paint(ColorFill.CYAN, ColorFill.BLACK));
		getSceneElements().add(
				EAdElementsFactory
						.getInstance()
						.getSceneElementFactory()
						.createSceneElement(asset31, asset32, x,
								margin));
		x += margin + size;
		...
		getSceneElements().add(
				EAdElementsFactory
						.getInstance()
						.getSceneElementFactory()
						.createSceneElement(asset51, asset52, x,
								margin));
		x += margin + size;

		// Random shape
		EAdDrawable asset41 = shapeFactory.getElement(
				ShapeFactory.ShapeType.IRREGULAR_RANDOM_SHAPE, size,
				size, new Paint(ColorFill.GRAY, ColorFill.BROWN));
		EAdDrawable asset42 = shapeFactory
				.getElement(
						ShapeFactory.ShapeType.IRREGULAR_RANDOM_SHAPE,
						size, size, new Paint(ColorFill.DARK_GRAY,
								ColorFill.BLACK));
		getSceneElements().add(
				EAdElementsFactory
						.getInstance()
						.getSceneElementFactory()
						.createSceneElement(asset41, asset42, x,
								margin));
		x = margin;

		// Drop shape
		EAdDrawable asset61 = shapeFactory.getElement(
				ShapeFactory.ShapeType.DROP_SHAPE, size, size,
		...
		EAdDrawable asset21 = shapeFactory.getElement(
				ShapeFactory.ShapeType.RECTANGULAR_SHAPE, size,
				size * 3,
				new Paint(ColorFill.RED, ColorFill.BROWN, 2));
		EAdDrawable asset22 = shapeFactory.getElement(
				ShapeFactory.ShapeType.RECTANGULAR_SHAPE, size,
				size * 3, new Paint(ColorFill.LIGHT_BROWN,
						ColorFill.DARK_BROWN, 10));
		SceneElement rotatingRectangle = EAdElementsFactory
				.getInstance().getSceneElementFactory()
				.createSceneElement(asset21, asset22, 330, 200);
		...
		InterpolationEf interpolation = EAdElementsFactory
				...
				.getInterpolationEffect(
						...
						InterpolationLoopType.RESTART,
						InterpolationType.LINEAR);
		SceneElementEv event = EAdElementsFactory
				.getInstance()
				.getEventsFactory()
				.getEvent(SceneElementEvType.FIRST_UPDATE,
						interpolation);
		rotatingRectangle.getEvents().add(event);
		getSceneElements().add(rotatingRectangle);

		SceneElement rotatingRectangle2 = new SceneElement(
				rotatingRectangle.getDefinition());
		rotatingRectangle2.setPosition(Corner.CENTER, 400, 300);
		ModifyHUDEf effect = new ModifyHUDEf(rotatingRectangle2, true);
		rotatingRectangle.addBehavior(MouseGEv.MOUSE_LEFT_PRESSED,
				effect);
		InterpolationEf interpolation2 = EAdElementsFactory
				...
				.getInterpolationEffect(
						...
						InterpolationLoopType.RESTART,
						InterpolationType.LINEAR);
		rotatingRectangle2.addBehavior(MouseGEv.MOUSE_ENTERED,
				interpolation2);
